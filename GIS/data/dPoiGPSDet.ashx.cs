...
namespace EBMSMap30.data
{
    ...
    /// </summary>
    public class dPoiGPSDet : IHttpHandler
    {
        DataTable tb,tb2;
        ...
        private void ExecDB(HttpContext context)
        {
            ...
            SqlDataAdapter SqlCmd = new SqlDataAdapter("spPOI_GetDetGps2", SqlConn);
            ...
            SqlCmd.SelectCommand.Parameters["@Page"].Value = context.Request["page"];
            ...
            if (tb.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < tb2.Rows.Count; i++)
                {
                    gpsdet.Datas.Add(new GpsDataSet()
                    {
                        ...
                        F = tb2.Rows[i]["F"].ToString(),
                        Sat = tb2.Rows[i]["Sat"].ToString(),
                        FuelR = string.Format("{0:0}",tb2.Rows[i]["FuelR"])
