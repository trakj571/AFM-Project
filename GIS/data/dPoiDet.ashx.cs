    ...
    public class dPoiDet : IHttpHandler
    {
        ...
        private void WriteJS(HttpContext context)
        {
            ...
            if (DS.Tables[0].Rows.Count > 0)
            {
                ...
                poiinfoset.FreqStat = GetFreqStat(context,DS.Tables[0].Rows[0]["PoiID"].ToString(),DS.Tables[0].Rows[0]["Name"].ToString());
        ...
        private string GetFreqStat(HttpContext context,string poiID,string poiName)
        {
            if (context.Request["rmt_lyids"]==null || context.Request["rmt_lyids"] == "")
            {
                return "";
            }
            ...
            SqlDataAdapter SqlCmd = new SqlDataAdapter("dms.spDMS_FreqStatPoi", SqlConn);
            SqlCmd.SelectCommand.CommandType = CommandType.StoredProcedure;
            ...
            string dt = context.Request["rmt_lyids"].Split('@')[1];
            string fDt = dt.Split(',')[0];
            string tDt = dt.Split(',')[1];
            ...
            sb.Append("<table style='border:1px solid #ccc;' class='text-center'>");
            sb.Append("<thead><tr><td style='border:1px solid #ccc;font-size:13px'>ลำดับที่</td>");
            sb.Append("<td style='border:1px solid #ccc;font-size:13px'>ย่านความถี่(MHz)</td>");
            sb.Append("<td style='border:1px solid #ccc;font-size:13px'>จำนวนช่องความถี่</td>");
            sb.Append("<td style='border:1px solid #ccc;font-size:13px'>Channel Spacing/BW</td>");
            sb.Append("<td style='border:1px solid #ccc;font-size:13px'>ปริมาณการครอบครอง</td></tr></thead><tbody>");

            for (int i = 0; i < tbD.Rows.Count; i++)
            {
                sb.Append("<tr>");
                sb.Append("<td style='border:1px solid #ccc;font-size:13px' data-th='ลำดับที่'>" + (i + 1) + "</td>");
                sb.Append("<td style='border:1px solid #ccc;font-size:13px' data-th='ย่านความถี่(MHz)'>" + tbD.Rows[i]["fFreq"] + "-" + tbD.Rows[i]["tFreq"] + "</td>");
                sb.Append("<td style='border:1px solid #ccc;font-size:13px' data-th='จำนวนช่องความถี่'>" + tbD.Rows[i]["nCh"] + "</td>");
                sb.Append("<td style='border:1px solid #ccc;font-size:13px' data-th='Channel Spacing/BW'>" + tbD.Rows[i]["ChSp"] + "</td>");
                sb.Append("<td style='border:1px solid #ccc;font-size:13px' data-th='ปริมาณการครอบครอง'>" + tbD.Rows[i]["Cnt"] + "</td>");
