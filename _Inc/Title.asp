...
<link rel="stylesheet" href="../_inc/css/fm-style.css?v=1.1">
...
<style>
    ...
    .content-readonly .afms-field {
        border-bottom: 0;
        margin: 0;
        margin-bottom: 10px;
    }

        .content-readonly .afms-field .afms-field-title {
            color: #922d35;
            font-weight: 500;
            margin-bottom: 5px;
        }

        .content-readonly .afms-field .afms-ic_calendar {
            bottom: 2px;
        }

    .content-readonly .afms-field_datepicker p:not(.nbtc-field-title) {
        padding-left: 24px;
    }
    ...
    .afms-pagination {
        ...
        .afms-pagination .is-active {
            border: 1px solid #922d35;
            background-color: #922d35;
            color: #ffffff !important;
        }

        .afms-pagination a:hover {
            color: #922d35;
            border: 1px solid #922d35;
        }

        .afms-pagination .afms-ic_firstpage,
        .afms-pagination .afms-ic_lastpage {
            font-size: 14px;
            padding-top: 8px;
            color: #000
        }
...
<script language="javascript">
    ...
    function delItem(fn, id, val, reload, isparent,url) {
        swal({
...
<script type="text/javascript">
    ...
    Response.Write("var IsFMSViewOnly = " + (uIdentity.Permission["IsFMSViewOnly"].ToString() == "Y" ? "true" : "false") + ";");
    Response.Write("var IsFMSEdit = " + (uIdentity.Permission["IsFMSEdit"].ToString() == "Y" ? "true" : "false") + ";");
    Response.Write("var IsFMSFMon = " + (uIdentity.Permission["IsFMSFMon"].ToString() == "Y" ? "true" : "false") + ";");
    Response.Write("var IsDMSImp = " + (uIdentity.Permission["IsDMSImp"].ToString() == "Y" ? "true" : "false") + ";");
    Response.Write("var IsDMSViewOnly = " + (uIdentity.Permission["IsDMSViewOnly"].ToString() == "Y" ? "true" : "false") + ";");
             
         %>
        $(function () {
            if (IsFMSFMon) $(".IsFMSFMon").show(); else $(".IsFMSFMon").hide();

            if (IsFMSEdit) $(".IsFMSEdit").show(); else $(".IsFMSEdit").hide();
            if (IsFMSViewOnly) $(".IsFMSViewOnly").show(); else $(".IsFMSViewOnly").hide();
            if (IsDMSImp) $(".IsDMSImp").show(); else $(".IsDMSImp").hide();
            if (IsDMSViewOnly) $(".IsDMSViewOnly").show(); else $(".IsDMSViewOnly").hide();
        });
</script>
